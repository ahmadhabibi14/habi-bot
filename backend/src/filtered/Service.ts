...
export async function addSektor(Sk: string, point: number) {
  try {
    ...
    sekName.rata_rata =
      (sekName.rata_rata * sekName.jumblah + point) / sekName.jumblah + 1;
    sekName.jumblah += 1;
...
export async function addWitel(Sk: string, point: number) {
  try {
    ...
    witName.rata_rata =
      (witName.rata_rata * witName.jumblah + point) / witName.jumblah + 1;
    witName.jumblah += 1;
...
export async function addReg(Sk: string, point: number) {
  try {
    ...
    regName.rata_rata =
      (regName.rata_rata * regName.jumblah + point) / regName.jumblah + 1;
    regName.jumblah += 1;
...
export async function upSektor(Sk: string, point: number, opr: "+" | "-") {
  try {
    ...
    if (opr == "+") {
      sekName.rata_rata =
        (sekName.rata_rata * sekName.jumblah + point) / sekName.jumblah;
    } else {
      sekName.rata_rata =
        (sekName.rata_rata * sekName.jumblah - point) / sekName.jumblah;
    }
...
export async function upWitel(Sk: string, point: number, opr: "+" | "-") {
  try {
    ...
    if (opr == "+") {
      witName.rata_rata =
        (witName.rata_rata * witName.jumblah + point) / witName.jumblah;
    } else {
      witName.rata_rata =
        (witName.rata_rata * witName.jumblah - point) / witName.jumblah;
    }
...
export async function upReg(Sk: string, point: number, opr: "+" | "-") {
  try {
    ...
    if (opr == "+") {
      regName.rata_rata =
        (regName.rata_rata * regName.jumblah + point) / regName.jumblah;
    } else {
      regName.rata_rata =
        (regName.rata_rata * regName.jumblah - point) / regName.jumblah;
    }
